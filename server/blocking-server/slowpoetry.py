...
def send_poetry(sock, poetry_file, num_bytes, delay):
    ...
    while True:
        ...
        try:
            #There is a method called sock.send(), not sure what's the difference
...
def main():
    ...
    #listen(backlog) backlog specifies the maximum number of queued connections
    sock.listen(5)

    print 'Serving %s on port %s.' % (poetry_file, sock.getsockname()[1])
    #This sock is used to listen, called listen_socket
